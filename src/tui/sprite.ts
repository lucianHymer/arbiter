...
export class Sprite {
   ...
   *
   * Only works if `controlled === true`. Plays footstep sound.
   *
   ...
   * Plays footstep sound on each step.
   *
   ...
   *
   * Plays magic sound. After ~400ms, sets visible=true and resolves.
   *
   ...
   *
   * Plays magic sound. After ~400ms, sets visible=false and resolves.
   *
   ...
   *
   * Plays magic sound. After ~400ms, changes the tile and resolves.
   *
   ...
   *
   * Plays drop sound (or quickNotice as fallback). Immediately sets visible=true.
   *
   * @returns Promise that resolves immediately after setting visible
   */
  async physicalSpawn(): Promise<void> {
    // 'drop' sound doesn't exist, use 'quickNotice' as fallback
    playSfx('quickNotice');
    this.visible = true;
  }
